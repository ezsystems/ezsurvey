...
{section var=option loop=$question.options}
    <div class="block"><label><input name="{$attr_name}" type="radio" value="{$option.value}"{if is_set($previous_vars.answer)}{if $previous_vars.answer|eq($option.value)} checked="checked"{/if}{else}{if is_set( $question_result.content[$option.value] )}{/if} checked="checked"{/if} onchange="synchFormElements( '{$attr_id}', '{$attr_alternative}', true );">{$option.label}</label></div>
{/section}
{if $question.extra_info.enabled|eq(1)}
     <div class="block"><label><input id="{$attr_id}" name="{$attr_name}" type="radio" value="{$question.extra_info.value}"{if is_set($previous_vars.answer)}{if $previous_vars.answer|eq($question.extra_info.value)} checked="checked"{/if}{else}{if is_set( $question_result.content[$question.extra_info.value])} checked="checked"{/if}{/if} onchange="synchFormElements( '{$attr_id}', '{$attr_alternative}', true );" />{$question.extra_info.label|wash(xhtml)}</label></div>
{/if}
...
{case match=1} {* Radio buttons in a row *}
{section var=option loop=$question.options}
    <div class="element"><label><input name="{$attr_name}" type="radio" value="{$option.value}"{section show=$option.toggled|eq(1)} checked="checked"{/section} onchange="synchFormElements( '{$attr_id}', '{$attr_alternative}', true );">{$option.label}</label></div>
{/section}
{if $question.extra_info.enabled|eq(1)}
...
{case match=2} {* Radio buttons in a column *}
{section var=option loop=$question.options}
    <div class="block"><label><input name="{$attr_name}" type="radio" value="{$option.value}"{section show=$option.toggled|eq(1)} checked="checked"{/section} onchange="synchFormElements( '{$attr_id}', '{$attr_alternative}', true );">{$option.label}</label></div>
{/section}
{if $question.extra_info.enabled|eq(1)}
